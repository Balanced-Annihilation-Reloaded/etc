...
local function CheckHardware()
  ...
  if (not canShader) then
    Spring.Echo("guishader api: your hardware does not support shaders")
    widgetHandler:RemoveWidget()
    return false
  end
...
function CreateShaders()
  ...
  local str_blurShader_part2 = [[
        ...
        gl_FragColor.rgb += 1.0/16.0 * texture2D(tex0, texCoord + vec2(-intensity, -intensity)).rgb;
        gl_FragColor.rgb += 2.0/16.0 * texture2D(tex0, texCoord + vec2(-intensity,  0.0)).rgb;
        gl_FragColor.rgb += 1.0/16.0 * texture2D(tex0, texCoord + vec2(-intensity,  intensity)).rgb;
        gl_FragColor.rgb += 2.0/16.0 * texture2D(tex0, texCoord + vec2( 0.0,    -intensity)).rgb;
