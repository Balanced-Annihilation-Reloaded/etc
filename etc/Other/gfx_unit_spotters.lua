function widget:GetInfo()
   return {
      name      = "Unit Spotters",
      desc      = "(modified widget: UnitShapes)",
      author    = "Floris",
      date      = "04.04.2014",
...
local OPTIONS = {
	...
	circleSpaceUsage				= 0.6,		-- 1 = whole circle space gets filled
	circleInnerOffset				= 0.4,
...
local function CreateDisplayLists(callback)
	...
	displayLists.invertedSelect = callback.fading(outersize, OPTIONS.selectinner)
		...
		for teamListIndex=1, numberOfTeams do
			...
			if #teamVisibleSelected > 0 then
				...
				if OPTIONS.showBase then
					...
					scale = 1.31 * OPTIONS.scaleMultiplier * animationMultiplier
