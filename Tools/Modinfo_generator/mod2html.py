...
def unitpage(id):
	...
	erow(outf)
	for key in sorted (units.iterkeys()):
		if 'buildoptions' in units[key] and id in units[key]['buildoptions'].values():
			srow(outf)
			tdunitimg(outf,key,key+' - '+units[key]['name'])
			erow(outf)
	...
	if 'explodeas' in units[id]:
		try:
			...
			print 'Failed to find explodeas weapon',units[id]['explodeas'], 'for',id,'in the table:', explosions.keys()
			
	if 'selfdestructas' in units[id]:
		try:
			writeweapon(outf,units[id]['selfdestructas'],explosions[units[id]['selfdestructas'].lower()])
		except:
			print 'Failed to find selfdestructas weapon',units[id]['selfdestructas'], 'for',id,'in the table:', explosions.keys()
